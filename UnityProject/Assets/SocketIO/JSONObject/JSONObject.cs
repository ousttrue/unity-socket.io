...
public class JSONObject {
    ...
    public static Action<Object> LogDebug = o => Console.WriteLine(o);
    public static Action<Object> LogInfo = o => Console.WriteLine(o);
    public static Action<Object> LogWarning = o => Console.WriteLine(o);
    public static Action<Object> LogError = o => Console.WriteLine(o);
	...
	void Parse(string str, int maxDepth, bool storeExcessLevels, bool strict) {
		if(!string.IsNullOrEmpty(str)) {
			...
			if(strict) {
				if(str[0] != '[' && str[0] != '{') {
					type = Type.NULL;
					LogWarning("Improper (strict) JSON formatting.  First character must be [ or {");
					return;
				}
			}
			if(str.Length > 0) {
					...
					switch(str[offset]) {
						...
						default:
							try {
								...
								type = Type.NULL;
								LogWarning("improper JSON formatting:" + str);
							}
							return;
					}
					string propName = "";
	...
	static void MergeRecur(JSONObject left, JSONObject right) {
		if(left.type == Type.NULL)
			...
			if(right.Count > left.Count) {
				LogError("Cannot merge arrays when right object has more elements");
				return;
			}
			for(int i = 0; i < right.list.Count; i++) {
		...
		if(type == Type.OBJECT) {
			...
			for(int i = 0; i < list.Count; i++) {
				...
				switch(val.type) {
					...
					default: LogWarning("Omitting object: " + keys[i] + " in dictionary conversion"); break;
				}
			}
			return result;
		}
		LogWarning("Tried to turn non-Object JSONObject into a dictionary");
		return null;
	}
